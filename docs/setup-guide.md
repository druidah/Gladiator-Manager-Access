...
When the user first interacts with the AI agent in this directory (e.g., "Hello", "New project", "Let's go"), conduct this interview.
...
- **"Not at all"**: Mark for tutorial text extraction after decompilation (see Step 7b). During feature planning, the AI agent should explain discovered game mechanics in more detail.
...
If no, install via WinGet (preferred — the AI agent can run this automatically):
...
Install the command-line tool via dotnet (preferred — the AI agent can run this automatically):
...
- **Advantage:** Fully command-line controlled, the AI agent can automate the entire decompilation
- Command-line usage: `ilspycmd -p -o decompiled "[Game]_Data\Managed\Assembly-CSharp.dll"`
- This makes the entire decompilation process automatable — the AI agent can do it for you
...
5. There, select the target directory (best to create a "decompiled" subdirectory in this project directory beforehand, so the AI agent can easily find the source code)
...
- Enter the concrete paths in the AI agent.md under "Environment"

#### Trim the AI agent.md after setup

Once `project_status.md` is created, trim the AI agent.md to save tokens for the rest of the project:
...
the AI agent re-reads the entire conversation every time you send a message. This means long conversations cost increasingly more tokens. To be efficient:
...
- **Before starting a new conversation:** the AI agent should always update `project_status.md` so the next conversation knows exactly where things stand.
- **When you come back:** Just say "hello" or "let's continue" - the AI agent reads `project_status.md` and picks up where you left off.
...
- **You don't need to repeat** the game name, the project setup, what was done before, or technical details. the AI agent reads `project_status.md` and knows all of that.
- **Reporting test results:** Just describe what happened naturally. "The menu works but item 3 is skipped" or "Nothing happens when I press F2" is enough. the AI agent will ask follow-up questions if needed.
- **Requesting features:** "Let's do the inventory next" or "Can we add health announcements?" — the AI agent checks the feature plan and starts working.
- **If something feels wrong:** "I think X broke since last time" — the AI agent will investigate.
...
```
Session start → the AI agent reads project_status.md → summarizes → asks what to do
  → You say what to work on (or the AI agent suggests)
  → the AI agents → builds → you test in game → report results
  → Repeat until feature is done
  → the AI agent updates project_status.md → suggests new session
```

### When things go wrong between sessions

- **Game updated and mod broke:** Tell the AI agent, it will check what changed
- **You forgot what was planned:** Just say "hello" — the AI agent tells you
- **You want to change direction:** Just say so, the AI agent adapts the plan
- **You lost your test notes:** the AI agent can rebuild context from `project_status.md` and the code
...
Every menu feature will need to read text from UI elements. If you document the access pattern once, you (and the AI agent) can reuse it everywhere without re-analyzing each time.
...
1. **Write/modify code** - the AI agent writes or modifies the mod source code
...
7. **Report back** - Tell the AI agent what worked and what didn't. Be specific: "It says X but should say Y" or "Nothing happens when I press F2"
8. **Repeat** - the AI agent fixes issues based on your feedback, then build and test again
...
- Build errors (compilation failures) are shown in the terminal - the AI agent can read and fix them directly
...
### Phase 2.5: Build/Deploy Scripts and the AI agent.md Update (after first successful build)

After the first successful manual build, create PowerShell scripts so that build and deploy are always one command away. Then update the AI agent.md to reference these scripts.
...
**Do NOT put raw `dotnet build` commands in the AI agent.md.** Always use the scripts — they are the single source of truth for how to build and deploy.
...
**Keep the AI agent.md short and concise** - it's only for the AI agent, not documentation.
...
wnload
/download
tps://dotnet.microsoft.com/download
